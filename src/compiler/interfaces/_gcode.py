...
class Gcode(Interface):
    ...
    def set_laser_power(self, power, minimum=0, maximum=1000):
        if power < 0 or power > 1:
            raise ValueError(f"{power} is out of bounds. Laser power must be given between 0 and 1. "
                             f"The interface will scale it correctly.")

        return f"M4 S{formulas.linear_map(minimum, maximum, power)};"
    ...
    def draw_point(
        ...
        point: Vector,
        drawing_power: int,
        drawing_point_time: int,

    ) -> str:
        command = [
            self.linear_move(point.x, point.y),
            self.set_laser_power(drawing_power),
            self.dwell(drawing_point_time),
            self.laser_off()
        ]
        command = "\n".join(command)
        return command
